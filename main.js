...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var playbackIndicator = document.getElementById('playbackIndicator');

    // Event listener for when the video ends
    video.addEventListener('ended', function() {
        // Clear the playback indicator text
        playbackIndicator.innerText = ''; // Clear the text content
    });
});

function setPlaybackSpeed(speed) {
    // Set the playback speed (implementation depends on your specific use case)
    console.log(`Setting playback speed to ${speed}`);
    
    // Get all buttons
    const buttons = document.querySelectorAll('#playbackButtons .image-button');
    
    // Remove 'active' class from all buttons
    buttons.forEach(button => button.classList.remove('active'));
    
    // Variable to hold the current speed text
    let speedText = null; // Initialize speedText as null
    
    // Add 'active' class to the clicked button and set the indicator text
    if (speed === 0.5) {
        document.getElementById('btnSlow').classList.add('active');
        speedText = 'Slow'; // Update speedText when the button is clicked
    } else if (speed === 1) {
        document.getElementById('btnNormal').classList.add('active');
        speedText = 'Normal'; // Update speedText when the button is clicked
    } else if (speed === 2) {
        document.getElementById('btnFast').classList.add('active');
        speedText = 'Fast'; // Update speedText when the button is clicked
    }

    // Update the playback indicator text only if speedText is not null
    if (speedText !== null) {
        playbackIndicator.innerText = `Current Speed: ${speedText}`;
        
        // Set a timeout to clear the playback indicator text after 10 seconds
        setTimeout(function() {
            playbackIndicator.innerText = ''; // Clear the text content after 10 seconds
